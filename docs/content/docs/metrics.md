...
```python
combra.metrics.frechet_distance(mu1, sigma1, mu2, sigma2, eps=1e-6) → float
```
...
- **mu1** (*ndarray*) – Mean activation vector of the first image set.
- **sigma1** (*ndarray*) – Covariance matrix of the first image set.
- **mu2** (*ndarray*) – Mean activation vector of the second image set.
- **sigma2** (*ndarray*) – Covariance matrix of the second image set.
- **eps** (*float, optional*) – Diagonal nudge applied when the covariance product is singular, so the matrix square root stays finite. Default: `1e-6`.
...
```python
combra.metrics.collect_images(folder) → list[Path]
```
...
```python
combra.metrics.compute_stats(folder, extractor, batch_size=50) → tuple[ndarray, ndarray, int]
```
...
- **batch_size** (*int, optional*) – Forward-pass batch size. Default: `50`.
...
```python
combra.metrics.compute_fid(real_folder, gen_folder, model_path=None, batch_size=50) → tuple[float, int, int]
```
...
- **real_folder** (*str*) – Path to the real image folder.
- **gen_folder** (*str*) – Path to the generated image folder.
- **model_path** (*str or None, optional*) – Path to an InceptionV3 ONNX model. When `None`, falls back to `COMBRA_INCEPTION_ONNX`, then `COMBRA_INCEPTION_URL`. Default: `None`.
- **batch_size** (*int, optional*) – Forward-pass batch size. Default: `50`.
...
```python
combra.metrics.load_rows(parquet_path) → list[dict]
```
...
```python
combra.metrics.index_by_name_step(rows) → dict
```
...
```python
combra.metrics.find_ethalon(sweep_dir) → Path
```
...
```python
combra.metrics.parquet_has_step(parquet_path, step) → bool
```
...
```python
combra.metrics.compute_metrics(real, fake) → tuple[float, ndarray, ndarray, ndarray]
```
...
- **real** (*dict*) – Single row from `load_rows` for the reference distribution.
- **fake** (*dict*) – Single row from `load_rows`, expected to have matching `meta.step`.
...
```python
combra.metrics.compare_folders(folder_paths, ethalon_path, class_map=None, steps=None,
                               coef=1000, verbose=True, fid_by_kimg=None) → list[dict]
```
...
- **class_map** (*dict[str, str] or None, optional*) – `{fake_class: real_class}` mapping when names don't match. Default: `None`.
- **steps** (*Iterable[float] or None, optional*) – Subset of steps to keep — others are skipped. Default: `None`.
- **coef** (*float, optional*) – Multiplier applied to `w_dist` in the printed table only (records keep raw values). Default: `1000`.
- **verbose** (*bool, optional*) – Print a per-row table. Default: `True`.
- **fid_by_kimg** (*dict[str, float] or None, optional*) – Optional `{kimg_key: fid}` map — prints a `[kimg=… FID=…]` header above each checkpoint's rows. Default: `None`.
...
```python
combra.metrics.compare_pairs(pairs, step, coef=1000, verbose=True, label_header='label') → list[dict]
```
...
- **coef** (*float, optional*) – Multiplier applied to `w_dist` in the printed table only. Default: `1000`.
- **verbose** (*bool, optional*) – Print a per-row table. Default: `True`.
- **label_header** (*str, optional*) – Column header used in the printed table for the first column. Default: `'label'`.
...
```python
combra.metrics.metrics_vs_n(folder, ethalon_path, class_map=None, step=5.0, allowed_ns=None) → list[dict]
```
...
- **class_map** (*dict[str, str] or None, optional*) – `{fake_class: real_class}` mapping when names don't match between folders. Default: `None`.
- **step** (*float, optional*) – Histogram step to filter rows on. Default: `5.0`.
- **allowed_ns** (*set[int] or None, optional*) – If given, restrict N values to this set — stray parquets at other sample sizes are skipped. Default: `None`.
...
```python
combra.metrics.convergence_stats(df_metrics, metrics, endpoints_by_kind, expected_points=None) → pandas.DataFrame
```
...
- **expected_points** (*dict[str, int] or None, optional*) – `{kind: expected_n_points_per_curve}`. When given, prints a warning if a curve has the wrong count. Default: `None`.
...
```python
combra.metrics.print_convergence_report(result, metrics, kinds, endpoints_by_kind,
                                        alpha=0.05, step=None, kind_display=None) → None
```
...
- **alpha** (*float, optional*) – Significance threshold used for the per-metric `rej/n` counts. Default: `0.05`.
- **step** (*float or None, optional*) – If provided, prefixed to the header line for traceability. Default: `None`.
- **kind_display** (*dict[str, str] or None, optional*) – Display names (e.g. `{'san': 'SAN', 'diffit': 'DiffiT'}`). Default: `None`.
...
```python
combra.metrics.summarize_metric_distribution(result, metric, kinds, resolutions) → dict[str, str]
```
...
```python
combra.metrics.plot_wdist_convergence_grid(records_by_panel, classes,
                                           ...
                                           panel_annotations=None, annot_kind_labels=None) → plotly.graph_objects.Figure
```
...
- **kind_labels** (*dict[str, str] or None, optional*) – `{kind: legend_label}`. Default: `None`.
- **grain_labels** (*dict[str, str] or None, optional*) – `{class: column_label}`. Default: `None`.
- **row_keys** (*list or None, optional*) – Ordered row identifiers. Defaults to sorted unique keys from `records_by_panel`. Default: `None`.
- **col_label_fn** (*callable or None, optional*) – Override `class → column label`. Default: `None`.
- **title_fn** (*callable or None, optional*) – Override `(row_key, class) → subplot title`. Default: `None`.
- **save_path** (*str or None, optional*) – PNG output path. `None` skips saving. Default: `None`.
- **png_meta** (*dict or None, optional*) – `{key: value}` written as PNG tEXt chunks. Default: `None`.
- **fonts** (*dict or None, optional*) – Override the `title/axis/tick/legend` font sizes. Default: `None`.
- **height_per_row** (*int, optional*) – Per-cell height in pixels. Default: `560`.
- **width_per_col** (*int, optional*) – Per-cell width in pixels. Default: `720`.
- **metric** (*str, optional*) – Record key to plot on the y-axis. Default: `'w_dist'`.
- **y_label** (*str, optional*) – Y-axis title. Default: `'W-dist'`.
- **zero_line** (*bool, optional*) – Draw a horizontal reference line at `y = 0`. Default: `False`.
- **panel_annotations** (*dict or None, optional*) – `{(row_key, kind): text}` annotations drawn inside the matching panel. Default: `None`.
- **annot_kind_labels** (*dict[str, str] or None, optional*) – Display labels used when rendering `panel_annotations`. Default: `None`.
...
```python
combra.metrics.plot_metric_distribution(result, metric, kinds, resolutions,
                                        kind_display=None, res_style=None, bin_step=1.5,
                                        x_range=None, save_path=None, png_meta=None,
                                        fonts=None, height=900, width=900) → plotly.graph_objects.Figure
```
...
- **kind_display** (*dict[str, str] or None, optional*) – Subplot titles. Default: `None`.
- **res_style** (*dict[int, dict] or None, optional*) – `{res: {'color': str, 'label': str}}`. Defaults to an auto-palette. Default: `None`.
- **bin_step** (*float, optional*) – Histogram bin width on the x-axis. Default: `1.5`.
- **x_range** (*tuple[float, float] or None, optional*) – Clamp the x-axis. `None` → data-driven. Default: `None`.
- **save_path** (*str or None, optional*) – PNG output path. Default: `None`.
- **png_meta** (*dict or None, optional*) – PNG tEXt metadata. Default: `None`.
- **fonts** (*dict or None, optional*) – Override `title/axis/tick/legend` font sizes. Default: `None`.
- **height** (*int, optional*) – Figure height in pixels. Default: `900`.
- **width** (*int, optional*) – Figure width in pixels. Default: `900`.
