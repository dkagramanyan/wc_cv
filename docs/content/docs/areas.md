...
```python
combra.areas.plot_polygons_area(data, saved_image_name, step, N, M, indices=None, save=False,
                                start=1, end=None, pixel=50/1000, font_size=20, s=60,
                                log_min_val=-8, min_area_num=10) → None
```
...
- **N** (*int*) – Grid rows when arranging multiple classes.
- **M** (*int*) – Grid columns when arranging multiple classes.
- **indices** (*list[int] or None, optional*) – Class subset to draw. Default: `None`.
- **save** (*bool, optional*) – Write the figure to disk. Default: `False`.
- **start** (*int, optional*) – Histogram slice start bound for the linear fit. Default: `1`.
- **end** (*int, optional*) – Histogram slice end bound for the linear fit. Default: `None`.
- **pixel** (*float, optional*) – Physical pixel size (e.g. `50/1000` for 50 nm pixels). Default: `50/1000`.
- **font_size** (*int, optional*) – Plot font size. Default: `20`.
- **s** (*int, optional*) – Marker size. Default: `60`.
- **log_min_val** (*float, optional*) – Log-density floor below which bins are discarded before the fit. Default: `-8`.
- **min_area_num** (*int, optional*) – Minimum count per bin to include in the fit. Default: `10`.

**Returns**

None. Renders the matplotlib figure and, when `save=True`, writes `polygons_area_<saved_image_name>_step_<step>.png`.
...
```python
combra.areas.plot_polygons_effective_radius(data, saved_image_name, step, N, M, indices=None, save=False,
                                            start=1, end=None, pixel=50/1000, font_size=20, s=60,
                                            log_min_val=-8, max_area_val=35) → None
```
...
- **max_area_val** (*float, optional*) – Upper bound on radius before the fit. Default: `35`.

**Returns**

None. Renders the matplotlib figure and, when `save=True`, writes `polygons_radius_<saved_image_name>_step_<step>.png`.
