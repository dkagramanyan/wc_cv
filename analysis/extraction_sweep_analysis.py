...
LSQ fit, the model-free reflex share, the fit's reflex share, the relative
residual, the pixel-grid peaks at 45 and 90 degrees, the
...
def mass_between(a, b, mus, sigmas, shares):
    tot = 0.0
    for mu, sg, w in zip(mus, sigmas, shares, strict=True):
        z = norm.cdf(360, mu, sg) - norm.cdf(0, mu, sg)
        tot += w * (norm.cdf(b, mu, sg) - norm.cdf(a, mu, sg)) / z
    return tot
...
for f in sorted(glob.glob(f"{SWEEP}/*_msl*_tol*/angles_n360.parquet")):
    ...
    for r in pq.read_table(f).to_pylist():
        ...
        mus, sg, shares = st["angles_gauss_mus"], st["angles_gauss_sigmas"], st["angles_gauss_shares"]
        curve = stats.truncated_bimodal_gaussian(x5, *mus, *sg, shares[0], 5.0)
        ...
        p_fit = mass_between(180, 360, mus, sg, shares)
        trough_obs = float(np.mean((theta >= 170) & (theta < 190)))
        trough_fit = mass_between(170, 190, mus, sg, shares)
        tail_obs = float(np.mean((theta < 20) | (theta >= 340)))
        tail_fit = mass_between(0, 20, mus, sg, shares) + mass_between(340, 360, mus, sg, shares)
        rows.append(
            dict(
                msl=msl, tol=tol, cls=name, n=theta.size, per_img=theta.size / n_img,
                mu1=mus[0], mu2=mus[1], s1=sg[0], s2=sg[1], p=shares[0],
                p_obs=p_obs, p_fit=p_fit, rel_res=rel_res,
        ...
        hists[(name, msl, tol)] = (x1, y1, x5, curve, mus, sg, shares)

hdr = "| class | msl | tol | n | per img | mu1 | mu2 | s1 | s2 | w (share 1) | p_obs | p_fit | rel res | pk45 | pk90 | top5 repeated | trough obs/fit | tail obs/fit |"
print(hdr)
print("|" + "---|" * (hdr.count("|") - 1))
for r in sorted(rows, key=lambda r: (r["cls"], r["msl"], r["tol"])):
    print(
        ...
        f"| {r['p']:.3f} | {r['p_obs']:.3f} | {r['p_fit']:.3f} | {r['rel_res']:.4f} | {r['pk45']:.2f} | {r['pk90']:.2f} | {r['top5']:.1%} "
...
for cls in sorted({r["cls"] for r in rows}):
    ...
    for i, msl in enumerate((5, 10, 15)):
        for j, tol in enumerate((2, 3, 5)):
            ax = axes[i, j]
            x1, y1, x5, curve, mus, sg, shares = hists[(cls, msl, tol)]
            ax.bar(x1, y1, width=1.0, color="0.8")
            xx = np.linspace(0, 360, 721)
            ax.plot(xx, stats.truncated_bimodal_gaussian(xx, *mus, *sg, shares[0]), "k-", lw=1.2)
